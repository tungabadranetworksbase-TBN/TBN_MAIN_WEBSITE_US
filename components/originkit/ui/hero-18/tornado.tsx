...
// Samples in each baked curve. Baked once, so this really does only trade
// smoothness for memory.
const CURVE_SAMPLES = 1024;

// Segments along each strand. This one is not free: the strand pass rewrites
// every segment of every strand on the CPU, so the frame cost is this times the
// strand count, and at 400 x 240 it was ninety-six thousand vertices laid out
// per rebuild — enough to hold the whole page at single-digit frames while the
// hero was on screen. 180 is one vertex per four screen pixels at the size this
// is drawn, which is still under what the curve needs to read as smooth.
const STRAND_SEGMENTS = 180;
...
// The strand set is rewritten a slice at a time, this many frames to a full
// pass. Two keeps the old thirty-a-second refresh on a 60Hz display while
// halving what any one frame has to lay out.
const STRAND_SLICES = 2;
...
function createVortex(
  ...
  /* ---------------------------------------- on screen, or not
   * The strand pass lays out every vertex on the CPU, which is the most
   * expensive thing this component does by a wide margin. It has no business
   * doing that while the hero is scrolled off the top of the page — which,
   * on a landing page, is nearly all of the scroll. The loop keeps ticking so
   * it picks straight back up, but does no work while nothing can be seen.
   * rootMargin buys a frame or two of warm-up before the hero is back. */
  let onScreen = true;
  ...
  function waveAt(
    ...
    for (let i = 0; i < MAX_WAVES; i++) {
      ...
      const dy = y - w.y;
      const dz = z - w.z;
      // Range test on the square first. This runs once per wave per strand
      // vertex — tens of thousands of times a frame — and all but a handful
      // are out of reach, so the root is only worth taking for the survivors.
      const sq = dx * dx + dy * dy + dz * dz;
      const reach = radius * 1.5;
      if (sq < 1e-6 || sq > reach * reach) continue;
  ...
  function step(now: number) {
    ...
    // Scrolled past: nothing to draw for, so nothing is drawn. `born` carries
    // the gap forward so the entrance does not count time nobody watched, and
    // resumes where it left off rather than snapping to the end.
    ...
    /* strands, a slice of the set per frame.
     *
     * Laying every strand out in one frame and then skipping the next is what
     * a rate limit does, and it is the wrong shape: the frame that does the
     * work overruns its 16ms and the display halves to 30, while the frame
     * that skips has nothing to do. Walking a slice of the strands each frame
     * refreshes the whole set at the same rate it did before and costs a
     * fraction of it per frame. Strands a slice apart are laid out one frame
     * apart, which at this flow is well under a pixel. */
    if (strandGeo && strands.length) {
      const slice = Math.ceil(strands.length / STRAND_SLICES);
      for (let i = 0; i < slice; i++) {
        drawStrand(strands[strandCursor], t, fadeStrand);
        strandCursor = (strandCursor + 1) % strands.length;
      }
  ...
  return {
    ...
    dispose() {
      ...
      for (const d of disposables) d.dispose();
      renderer.dispose();
      // The context is deliberately NOT force-lost. Losing it is permanent for
      // the canvas element it belongs to, and React remounts this effect onto
      // that same element — StrictMode does it on every dev mount — so the
      // next createVortex would be handed the dead context and throw. Dropping
      // the GL objects is the part that matters; the context itself goes when
      // the canvas is collected.
      // ponytail: relies on GC to reclaim the context. One hero, one canvas,
      // so it never approaches the per-page context cap. If a page ever mounts
      // several of these, give each mount its own canvas element and force the
      // loss again.
    },
  };
}
...
export default function Vortex(props: VortexProps) {
  ...
  // A canvas whose WebGL context never came up — or died later — does not
  // paint transparent, it paints an opaque box. Left in place over a hero it
  // covers the artwork and the copy behind it, so the element has to leave
  // the page entirely rather than merely stop drawing.
  ...
  React.useEffect(() => {
    ...
    const onContextLost = () => setDead(true);
    ...
    try {
      ...
      // render). Drop the canvas and let the section's own background show,
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        style={{ display: dead ? "none" : "block", width: "100%", height: "100%" }}
      />
    </div>
  );
}